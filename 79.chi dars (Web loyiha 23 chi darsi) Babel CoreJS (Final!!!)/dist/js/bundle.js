  ...
  class MenuCard {
    ...
    render() {
      ...
      if (this.classes.length === 0) {
        ...
        this.classes.forEach(classname => element.classList.add(classname)); //yani rest operator yordamida classlar diynamic qo'shildi yani rest operator(...)bor classes parametri massiv qaytaradi shu classes parametrini foreach bilan intirgatsa qilib element o'zgaruvchi ichidagi classnamelarga add qilib classnamelarni qo'shdik endi new MenuCardlardagi ohirgi parametr hissoblangan "menu__item" classi qayerga chaqirilsaham doim yangi classlarni qo'shadi
      }
      element.innerHTML = `
...
function form(formSelector, modalTimerId) {
  ...
  const msg = {
    ...
    //yani bu holatda serverdan keladigan javob 200 javobi kelguncha img papkani ichida spinner svg file ishlab turadi
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTrigger.forEach(item => {
    ...
    item.addEventListener("click", () => openModal(modalSelector, modalTimerId)); //yani bitta click hodisasini barcha data-modal atributibor elementlarga berib chiqdik va keyinchalik htmlda faqat bitta joyda data atributni qoldirdik???=> sababi bir nechta buttongaham bitta funksiyasni ishlatib ko'rdik
  });
  ...
  function showModalByScroll() {
    ...
    window.scrollY + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
...
function slider(_ref) {
  ...
  const slides = document.querySelectorAll(slide),
    //massiv qaytaradi yani offer__slide classlari 5 ta //htmlda offer ona divini ichida 5 ta odder__slide classlari bor ichida 5 ta card bor img descrepshini bilan
    ...
    //yani offer__slider-wrapper ona divini ichidagi ikkinchi ona div yani 5 ta carousel qilinishi kerak bo'lgan cardaalarni birinchi ona divi
    width = window.getComputedStyle(slidesWrapper).width,
    //yani htmldan slidesWrapper o'zgaruvchida chaqirilgan offer__slider-wrapper classini widthi olindi yani carouselga aloqador ona divni ovoldik
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function tab(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass //script78.jsdagi tab funksiyasi chaqirilganda argumentiga berilgan parammetrlar shu joyda yasavolindi yani nima vazifa bajarishi shu joyda yozib qo'yildi va tab.js chaqirilganda bu parametrlar tab.jsni argumentiga berib qo'yildi SHUNDA tab.jsni PARAMETRIGA    CLASSLAR QO'YILSAHAAM BU TAB FUNSKSIYASI O'SHA BOSHQA CLASSLARGAHAM ISHLAYVERADI YANI tab FUNSKIYASI DEYARLI UNVERSAL QILINDI!!!
) {
  const tabsParent = document.querySelector(tabsParentSelector),
    //htmldagi preview katta ona dividagi tabheader__items bola divi chaqirib olindi yani bu bola div saytdagi  Category Of Series bolimi hissoblanadi Category Of Series bo'limi esa preview bo'limini boshi yani catrgoryalarni tanlash bo'limi hissoblanadi
    tabs = document.querySelectorAll(tabsSelector),
    //tabheader__item classli divlar categoreylarni nomi yani comedy sport fantastic bo'limlarga olib o'tish uchun kerak bo'ladigan html documentlar chaqirib olindi
...
function timer(id, deadLine) {
  ...
  function setClock(selector, endtime) {
    ...
    const timer = document.querySelector(selector),
      //yani parametrdagi selector ona div bo'lgani uchun ichidagi hamma html elementlarni olish mumkun
...
async function postData(url, data) {
  //url bu holatda dynamic hissoblanadi//unversal funksya faqat url datalarga bo'gliq yani aynan qandaydur linkka emas yani bu funskiya chaqirilganda parametriga alohida linklarni berib bu funksiyani yana ishlatish mumkun???
  const res = await fetch(url, {
    ...
    method: "POST",
    ////yani fetchni post metodidan foydalanish bu metod post qilishni fetchdan post qilish yani formga post,yani userni post qilishini nazarda tutadi
    headers: {
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  ////webpacke kutubhonasi bilan script79.js filedagi loyihani uzun kodini har bir ishga aloqador funksiyalarni alohida alohida modullarga ajratib chiqildi endi dastur real loyihaga o'hshadi yani webpacke yuklanganda dist papka bilan keladi va disk papkada webpacke kutubhonasi bundle jsda hamma kodlarni tartiblab ishlatib beradi va asosiy script79.js fileda yuqoridagi kabi chaqiriladi yani require funksiyasi bilan chaqiriladi window.addEventListener("DOMContentLoaded" ichiga yozilishini sababi endi hamma kodlar modulga ajratilgan bo'lsada shu window.addEventListener("DOMContentLoaded"ni ichida ishlasa doimgiday ishlayveradi yani birinchi html csslar ochilib bo'lgandan keyingina ichidagi modullarga bo'lingan kodlarni ishlatadi
